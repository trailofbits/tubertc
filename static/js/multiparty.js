...
var reshapeThumbs = [
    function(parentw, parenth) {
        ...
        if( activeBox > 0 ) {
            return setThumbSize(0.10, 0.01, 0.01, parentw, parenth);
        }
        else {
            ...
            switch(connectCount) {
                ...
                case 4:
                case 5:
                    return reshape1of5(parentw, parenth);
            }
        }
    },
    function(parentw, parenth) {
        if( activeBox >= 0 || !boxUsed[1]) {
            return setThumbSize(0.10, 0.17, 0.01, parentw, parenth);
        }
        else{
            switch(connectCount) {
                case 1:
    ...
    function(parentw, parenth) {
        if( activeBox >= 0 || !boxUsed[2] ) {
            return setThumbSize(0.10, 0.33, 0.01, parentw, parenth);
        }
        else  {
            switch(connectCount){
                case 1:
    ...
    function(parentw, parenth) {
        if( activeBox >= 0 || !boxUsed[3]) {
            return setThumbSize(0.10, 0.5, 0.01, parentw, parenth);
        }
        else{
            switch(connectCount){
                case 1:
    ...
    function(parentw, parenth) {
        if( activeBox >= 0 || !boxUsed[4]) {
            return setThumbSize(0.10, 0.67, 0.01, parentw, parenth);
        }
        else {
            switch(connectCount) {
                case 1:
    ...
    function(parentw, parenth) {
        if( activeBox >= 0 || !boxUsed[5]) {
            return setThumbSize(0.10, 0.83, 0.01, parentw, parenth);
        }
        else {
            switch(connectCount) {
                case 1:
...
function muteMicButtonReshaper(parentw, parenth) {
    var imagew = 16;
    var imageh = 16;
    return setThumbSizeButton(0.05, -0.01, -0.01, parentw, parenth, imagew, imageh);
}
...
function muteMic() {
    var muteMicBtn = document.getElementById('muteMicButton');
    if( micIsMuted ) {
        muteMicBtn.src = "/images/mic-off.svg";
        micIsMuted = false;
    }
    else {
        muteMicBtn.src = "/images/mic-on.svg";
        micIsMuted = true;
    }
    easyrtc.enableMicrophone(!micIsMuted);
}

function callEverybodyElse(roomName, otherPeople) {
    if( otherPeople ) {
        console.log("Calling " + otherPeople.length + " other clients in " + roomName);
    }
    else {
        console.log("Only one in chat");
    }
...
function loginSuccess() {
    ...
    document.getElementById('muteMicButton').style.display = 'block';
}
...
function appInit(roomName) {
    easyrtc.joinRoom(roomName, null, function () {
        ...
        easyrtc.showError("SYSTEM-ERROR", "Failed to join room: " + roomName);
    });
    ...
    setReshaper('muteMicButton', muteMicButtonReshaper);
    ...
    easyrtc.setOnCall( function(easyrtcid, slot) {
        ...
        if(activeBox == 0 ) { // first connection
            ...
            //TODO: do not enable the text button for now..
            document.getElementById('textEntryButton').style.display = 'none';
        }
